 jQuery(document).ready(function($) {
    ...
    $(".scroll a, .navbar-brand, .gototop,.explore").click(function(event){   //最后一个.explore内容是空的可能是预留的
		event.preventDefault();// 方法阻止元素发生默认的行为（例如，当点击提交按钮时阻止对表单的提交）
		$('html,body').animate({scrollTop:$(this.hash).offset().top}, 600,'swing');//http://www.w3school.com.cn/jquery/effect_animate.asp
		$(".scroll li").removeClass('active');//滚动到最上边并清除导航栏所有活动标签
		$(this).parents('li').toggleClass('active');//上边清空的基础上如果点击的导航栏的某个项添加活动标签
    });
});
...
//http://code1.okbase.net/codefile/jquery.touchSwipe-1.2.5.js_2013011420208_5.htm
$('.carousel').swipe( {//滑动加轮播swipe为jquery mobile事件
     swipeLeft: function() {
         $(this).carousel('next');//Bootstrap 轮播（Carousel）插件
     },
     swipeRight: function() {
         $(this).carousel('prev');
     },
